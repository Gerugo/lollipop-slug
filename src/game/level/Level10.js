...
export class Level10 {
  constructor() {
    ...
    this.platforms = JSON.parse(JSON.stringify(LEVEL_10_CONFIG.platforms));
  ...
  drawBackground(ctx, camera) {
    ...
    // LAYER 1 (Distant Parallax 0.08): Grand Throne Sanctum & Cosmic Rosette ('cielo10')
    const skyImg = imageLoader.getImage('cielo10');
    ...
    // LAYER 2 (Midground Parallax 0.28): Imperial Golden Arches ('sanctum')
    const midImg = imageLoader.getImage('sanctum');
    ...
    for (let i = 0; i < 24; i++) {
      const sx = ((i * 59 + this.animTime * 25) % (viewW + 40)) - 20;
      const sy = ((i * 37 + this.animTime * 50) % (viewH - 40)) + 20;
      const sr = (i % 3) + 1.5;
      ctx.beginPath();
  ...
  drawContinuousGround(ctx, camera) {
    ...
    for (const plat of groundPlatforms) {
      ...
      // Imperial Gold & Ruby Sugar Frosting Highlight
      ctx.beginPath();
