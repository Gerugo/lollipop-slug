...
export class Level7 {
  ...
  drawBackground(ctx, camera) {
    ...
    // LAYER 1 (Distant Parallax 0.08): Neon Gummy Cavern Horizon ('cielo7')
    const skyImg = imageLoader.getImage('cielo7');
    ...
    // LAYER 2 (Midground Parallax 0.28): Translucent Gummy Arches ('gelatina')
    const midImg = imageLoader.getImage('gelatina');
    ...
    for (let i = 0; i < 18; i++) {
      const sx = ((i * 73 + this.animTime * 25) % (viewW + 40)) - 20;
      const sy = ((i * 47 + Math.sin(this.animTime * 2 + i) * 20 + i * 30) % (viewH - 80)) + 40;
      const sr = (i % 3) + 2;
      ctx.beginPath();
